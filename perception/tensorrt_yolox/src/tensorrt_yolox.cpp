  ...
  const size_t max_workspace_size, const std::string & color_map_path)
{
  ...
  if (precision == "int8") {
    ...
    if (use_dacup_) {  // Allocate buffer for backbonefeature
      std::string fnd = ".onnx";
  ...
  if (multitask_) {
    ...
    if (use_dacup_) {
      ...
      dacup_segmentation_in_h_ = cuda_utils::make_unique_host<float[]>(
        batch_config[2] * dacup_segmentation_in_size, cudaHostAllocPortable);
...
void TrtYoloX::dacup_preprocessGpu(const std::vector<cv::Mat> & images)
{
  ...
  resize_bilinear_nhwc_to_nchw32_batch_gpu(
    ...
    images[0].rows, 3, batch_size, static_cast<float>(1.0 / 255.0), *stream_);
  ...
  [[maybe_unused]] std::vector<cv::Mat> & color_masks)
{
  ...
  bool res;
  if (needs_output_decode_) {
    res = feedforwardAndDecode(images, objects, masks, color_masks);
  } else {
    res = feedforward(images, objects);
  }

  // ObjectArrays unknown_objects;
  if (use_dacup_) {
    dacup_preprocessGpu(images);
    res = dacup_feedforwardAndDecode(images, objects, masks, color_masks);
  }
  return res;
}
  ...
  [[maybe_unused]] std::vector<cv::Mat> & color_masks)
{
  ...
  CHECK_CUDA_ERROR(cudaMemcpyAsync(
    ...
    cudaMemcpyDeviceToHost, *stream_));
  CHECK_CUDA_ERROR(cudaMemcpyAsync(
    dacup_segmentation_in_h_.get(), dacup_segmentation_in_d_.get(),
    sizeof(float) * segmentation_out_elem_num_, cudaMemcpyDeviceToHost, *stream_));
  ...
  for (size_t i = 0; i < batch_size; ++i) {
    ...
    size_t out_elem_num = std::accumulate(
      output_dims.d + 1, output_dims.d + output_dims.nbDims, 1, std::multiplies<int>());
    out_elem_num = out_elem_num * batch;
    int out_w = static_cast<int>(output_dims.d[3]);
    int out_h = static_cast<int>(output_dims.d[2]);

    // scoremap mask
    cv::Mat scoremap_mask;

    float * segmentation_results =
      dacup_segmentation_in_h_.get() + (i * segmentation_out_elem_num_per_batch_);
    size_t counter_seg = 0;
    const auto output_dims_seg =
      trt_common_->getBindingDimensions(trt_common_->getBindingIndex("segmenter"));
    ...
    ObjectArray object_array;
    scoremap_mask = getAnomalyBox(
      &(segmentation_results[counter_seg]), &(anomaly_results[counter]), object_array, image_size,
      scales_[i], output_dims, out_w, out_h);

    anomaly_masks_.push_back(scoremap_mask);
    // out_masks.at(i) = anomaly_masks_.at(0);
    objects.emplace_back(object_array);
